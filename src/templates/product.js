...
const Product = (props) => {
    ...
    const description = renderRichText(props.data.contentfulShop.description, options)
    ...
    return (
        <Layout>
            ...
            <div className="product">
                <ImageSlider
                    photo={props.data.contentfulShop.photos}
                />
                <div className="product__content">
                    <Timer date={props.data.contentfulShop.timeout} />
                    <p className="product__content product__content--price">{props.data.contentfulShop.price} PLN</p>
                    <p className="product__content product__content--quantity">
                        Sztuk:
                        <button onClick={minus} disabled={quantity === 1}>-</button>
                        {quantity}
                        <button onClick={add} disabled={quantity === props.data.contentfulShop.quantity}>+</button>
                    </p>
                    <button
                    ...
                    <div>
                        <h2 className="product__content product__content--h2">{props.data.contentfulShop.title}</h2>
                        <p>{description}</p>
                        <p className="product__content product__content--env">
                            <FaLeaf className="product__content product__content--envicon" />Przyjazne środowisku
                        </p>
                        <p className="product__content product__content--height">
                            <IoMdResize className="product__content product__content--heighticon" />Wysokość: {props.data.contentfulShop.height} cm
                        </p>
                        <p className="product__content product__content--del">
                            <FaTruckMoving className="product__content product__content--delicon" />Darmowa wysyłka
                        </p>
                    </div>
                </div>
            </div>
